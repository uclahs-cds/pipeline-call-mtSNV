...
# TODO: create a function to get the actaul availiable partitions on our Slurm cluster
PARTITION_TYPES = ['lowmem', 'midmem', 'execute']
SCRATCH_SPACE = '/scratch'

# TODO: add timeout when cluster upgrades to python 3
...
def execute_sbatch_command(submission_command):
    ...
    try:
        ...
        process = subprocess.Popen(submission_command,
                                   shell=True,
...
# TODO: remove install command once Java and Nextflow are installed fully on SGE
def generate_qsub_command(install_requirements_command,
                          nextflow_run_command,
                          pipeline_run_name,
                          partition_type,
                          smp_slots,
...
# TODO: remove once Java and Nextflow are installed fully on SGE
def generate_install_requirements_command():
    ...
    install_nextflow_command = 'sudo wget -qO- ' \
        'https://github.com/nextflow-io/nextflow/releases/download/v20.07.1/nextflow-20.07.1-all ' \
        '| bash'
...
def generate_slurm_multi_node_nextflow_run_command(nextflow_run_command,
                                                   pipeline_run_name,
                                                   email=None):
    """
...
# submit the pipeline using the generated sbatch command
def submit_pipeline_run(multi_node_mode,
                        sge_scheduler,
                        nextflow_script,
                        pipeline_run_name,
                        smp_slot=None,
    ...
    scheduler_command = ""
    nextflow_run_command = generate_nextflow_run_command(nextflow_script, nextflow_config)

    if multi_node_mode:
        # make sure pipeline is run in background mode
        scheduler_command = generate_slurm_multi_node_nextflow_run_command(nextflow_run_command,
                                                                           pipeline_run_name,
                                                                           email)
    ...
    # if single node use
        ...
        if sge_scheduler:
            ...
            scheduler_command = generate_qsub_command(install_requirements_command,
                                                      nextflow_run_command,
                                                      pipeline_run_name,
                                                      partition_type,
                                                      smp_slot,
        ...
        # create command with sbatch to submit to Slurm
            ...
            scheduler_command = generate_sbatch_command(nextflow_run_command,
                                                        pipeline_run_name,
                                                        partition_type,
                                                        email)
...
def validate_input_combinations(multi_node_mode,
                                sge_scheduler,
                                partition_type=None,
                                smp_slots=None):
    """
    ...
    if multi_node_mode:
        multi_node_invalid_input_options = {'partition_type': partition_type,
                                            'smp_slots': smp_slots,
    ...
    # if single node use
        ...
        if sge_scheduler:
            single_node_required_sge_input_options = {'partition type': partition_type,
                                                      'smp_slots': smp_slots}
            validate_required_inputs(single_node_required_sge_input_options, 'SGE')

        # if submitting to Slurm
        else:
            single_node_invalid_slurm_input_options = {'smp_slots': smp_slots,
                                                       'sge_scheduler': sge_scheduler}
...
def get_pipeline_run_inputs():
    ...
    validate_input_combinations(args.multi_node_mode,
                                args.sge_scheduler,
                                args.partition_type,
                                args.smp_slots)
...
if __name__ == '__main__':
    ...
    sys.exit(submit_pipeline_run(ARGS.multi_node_mode,
                                 ARGS.sge_scheduler,
